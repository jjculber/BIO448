...
public class PalindromeDialog extends JDialog {
	/*
	 * CONSTANTS
	 */
	...
	private JTextField mFasta, mRangeBegin, mRangeEnd, mGapLengthMin, mGapLengthMax, mLengthMin;

	public PalindromeDialog(Frame owner, String title) {
		super(owner, title);
		...
		mGapLengthMin = new JTextField(20);
		mGapLengthMax = new JTextField(20);

		mLengthMin = new JTextField(20);

	}

	public static void main(String[] args) {
		PalindromeDialog dialog = new PalindromeDialog(null, "Find Repeats");
	...
	public void init() {
		...
		JPanel gapLengthMin = prepareRangeField("Gap Length Min:", mGapLengthMin);
		JPanel gapLengthMax = prepareRangeField("Gap Length Max:", mGapLengthMax);

		JPanel gapLengthField = new JPanel();
		gapLengthField.setLayout(new FlowLayout(FlowLayout.LEADING));
		gapLengthField.add(gapLengthMin);
		gapLengthField.add(gapLengthMax);
		
		JPanel lengthMin = prepareRangeField("Length Min:", mLengthMin);

		JPanel lengthField = new JPanel();
		lengthField.setLayout(new FlowLayout(FlowLayout.LEADING));
		lengthField.add(lengthMin);
		...
		mPane.add(gapLengthField);
		
		mPane.add(lengthField);
		...
		okayButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (mFasta.getText().equals("")) {
					...
					int rangeStart, rangeEnd, gapMin, gapMax, minLength;

					try {
						rangeStart = Integer.parseInt(mRangeBegin.getText());
					} catch (Exception ex) {
						rangeStart = 0;
					}

					try {
						rangeEnd = Integer.parseInt(mRangeEnd.getText());
					} catch (Exception ex) {
						rangeEnd = 0;
					}

					try {
						gapMin = Integer.parseInt(mGapLengthMin.getText());
					} catch (Exception ex) {
						gapMin = 0;
					}

					try {
						gapMax = Integer.parseInt(mGapLengthMax.getText());
					} catch (Exception ex) {
						gapMax = 0;
					}

					try {
						minLength = Integer.parseInt(mLengthMin.getText());
					} catch (Exception ex) {
						minLength = 0;
					}

					// TODO call palindrome code
