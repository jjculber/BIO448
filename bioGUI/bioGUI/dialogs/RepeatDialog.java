...
public class RepeatDialog extends JDialog {
	...
	private JTextField mFasta, mRangeBegin, mRangeEnd, mLengthMin, mLengthMax;
	private JCheckBox mMaxOnly;

	public RepeatDialog(Frame owner, String title) {
		super(owner, title);
		...
		mLengthMax = new JTextField(20);

		mMaxOnly = new JCheckBox();
	}

	public static void main(String[] args) {
		RepeatDialog dialog = new RepeatDialog(null, "Find Repeats");
		...
		okayButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (mFasta.getText().equals("")) {
					...
					try {
						min = Integer.parseInt(mLengthMin.getText());
					} catch (Exception ex) {
						min = 0;
					}

					try {
						max = Integer.parseInt(mLengthMax.getText());
					} catch (Exception ex) {
						max = 0;
					}
